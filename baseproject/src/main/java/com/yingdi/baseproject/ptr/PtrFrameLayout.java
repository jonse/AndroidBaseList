...
import com.yingdi.baseproject.util.PtrLocalDisplay;
...
public class PtrFrameLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (DEBUG && DEBUG_LAYOUT) {
            CLog.d(LOG_TAG, "onMeasure frame: contentWidth: %s, contentHeight: %s, padding: %s %s %s %s",
        ...
        if (mContent != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                ...
                CLog.d(LOG_TAG, "onMeasure content, contentWidth: %s, contentHeight: %s, margin: %s %s %s %s",
    ...
    private void updatePos(int change) {
        ...
        if (mStatus == PTR_STATUS_PREPARE) {
            // reach fresh contentHeight while moving from top to bottom
            if (isUnderTouch && mAutoScrollRefreshTag == 0 && mPullToRefresh
                    && mPtrIndicator.crossRefreshLineFromTopToBottom()) {
                tryToPerformRefresh();
            }
            // reach header contentHeight while auto refresh
